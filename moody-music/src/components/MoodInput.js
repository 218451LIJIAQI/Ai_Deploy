...
const getApiService = async () => {
  ...
  if (window.location.hostname.includes('netlify.app') || 
    ...
    console.log('🎵 MusicAI: Using production API service for Netlify');
    const { fetchSongsByMood } = await import('./apiService.production');
    return fetchSongsByMood;
  } else {
    console.log('🎵 MusicAI: Using standard API service');
    const { fetchSongsByMood } = await import('./apiService');
    return fetchSongsByMood;
  }
};

// ✅ Production API URL - matches your Render deployment
const API_BASE_URL = 'https://ai-deploy-2.onrender.com/api';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (inputMethod === 'dropdown') {
        ...
        console.log('🎵 MusicAI: Processing text mood detection...');
        ...
        console.log('🎵 MusicAI: Processing image emotion detection...');
        ...
        console.log(`🎵 MusicAI: Detected emotion: ${emotion}, recommended music type: ${mood}`);
      }
      ...
      // 🎵 使用智能 API 服务选择
      const fetchSongsByMood = await getApiService();
      const songs = await fetchSongsByMood(mood);
      
      console.log(`🎵 MusicAI: Successfully fetched ${songs.length} songs for mood: ${mood}`);
      onSongsFetched(songs);
      
    } catch (err) {
      console.error('🎵 MusicAI: Error in mood processing:', err);
      
      // 🎵 更友好的错误处理
      let errorMessage = 'Failed to detect mood or fetch songs.';
      
      if (err.message && err.message.includes('Spotify')) {
        errorMessage = `🎵 Spotify API Error: ${err.message}. Please check if Spotify credentials are configured correctly.`;
      } else if (err.response && err.response.status) {
        errorMessage = `🎵 API Error (${err.response.status}): ${err.response.data?.message || 'Unknown error occurred'}`;
      } else if (err.message) {
